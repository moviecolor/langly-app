# Langly — App Store Metadata

## App Name
**Langly** — AI Language Learning

## Subtitle (30 chars max)
Learn Languages Faster

## Categories
- **Primary:** Education
- **Secondary:** Reference

## Price
Free to download — **7-day free trial**, then **Langly Premium** subscription
- USA: **$8.99 / month**
- Brazil: **R$ 26,90 / month**
- All features included — no partial access, no locked modules

---

## App Description

**Learn any language with AI-powered lessons tailored to you.**

Langly is your personal language learning assistant that adapts to your pace and style. Whether you're a complete beginner or brushing up on skills, Langly makes language learning engaging, effective, and fun.

**VOCABULARY BUILDER**
• Play the Match Madness word game — match pairs before time runs out
• Audio Mode — listen on repeat to memorize, set reps and gap
• Track your progress with detailed scorecards
• Add up to 10 blocks of 15 words each

**AUDIO PRACTICE**
• Listen on repeat to memorize — set repetitions and gap
• Native pronunciation with text-to-speech
• Hands-free learning loop: English → target language → repeat
• The real learning tool — practice makes permanent

**GAMIFIED LEARNING**
• Earn points and track your streak
• Visual progress charts show your improvement
• Compete with yourself to beat high scores
• Unlock achievements as you master new vocabulary

**SMART & ADAPTIVE**
• AI-powered lesson generation keeps content fresh
• Personalized difficulty based on your performance
• Offline-ready — learn anywhere without internet
• Clean, focused interface with no distractions

**PRIVACY FIRST**
• All learning happens on your device
• No data collection, no tracking
• Your progress stays private

**LANGLY PREMIUM**
• Audio Mode — repetition-based practice loops for vocabulary
• Common Sentences — everyday phrases and expressions
• Pronunciation — real-time speaking feedback
• Q&A — interactive conversations
• 7-day free trial, then $8.99/month
• No ads, works fully offline

Download Langly today and start your language learning journey.

---

## Keywords (100 chars max, comma-separated)
language,learning,spanish,french,german,japanese,mandarin,vocabulary,flashcards,ai,education,study,pronunciation,grammar,translate

## What's New (v1.0)
• Vocabulary builder with AI-powered lessons
• Audio practice mode with native pronunciation
• Spaced repetition for optimal retention
• Score tracking and progress visualization
• Offline learning support

---

## Rating
**4+** — No objectionable content

## Age Rating
**4+**

## Copyright
© 2026 Langly App. All rights reserved.

## App Privacy
**Data Not Collected**
The developer does not collect any data from this app.

---

## App Store Connect Settings

### App Information
- Name: Langly
- Subtitle: Learn Languages Faster
- Category: Education
- Content Rights: Does not contain third-party content
- Age Rating: 4+

### Pricing and Availability
- Model: Free with subscription (Langly Premium)
- USA: $8.99 / month; Brazil: R$ 26,90 / month
- Available in all territories

### In-App Purchases (set up in App Store Connect)

| Product ID | Type | Price | Description |
|-----------|------|-------|-------------|
| com.langly.app.premium.monthly | Auto-Renewable Subscription | USA $8.99 / BRA R$ 26,90 | Langly Premium — unlocks all modules |

### Build
- Upload latest build via Xcode or `xcodebuild archive`
- Select build in App Store Connect
- Add build to version

### Screenshots Required
- iPhone 6.7" (iPhone 15 Plus/16 Plus) — 1290 × 2796
- iPhone 6.5" (iPhone 11 Pro Max/XS Max) — 1242 × 2688
- iPad Pro 12.9" (6th gen) — 2048 × 2732 (optional, recommended)

### App Preview Video (optional but recommended)
- Max 30 seconds
- Max 500 MB
- H.264, 1080p or 4K

---

## Privacy Policy
Required for App Store submission. Host at a public URL.
Template: See `PRIVACY_POLICY.md`

## Support URL
Create a simple support page or email: support@langly.app

## Marketing URL (optional)
https://langly.app
